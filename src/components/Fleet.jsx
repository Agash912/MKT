    ...
    <section id="fleet" className="py-14 lg:py-20 border-t border-white/[0.04]">
      <div className="max-w-[1100px] mx-auto px-5">
        <div className="grid grid-cols-2 gap-8 lg:gap-16 items-start">
          ...
          >
            <p className="text-[#C9A800] text-[11px] font-semibold tracking-[0.25em] uppercase mb-3">Our Fleet</p>
            ...
            <button onClick={() => go('#quote')}
              className="text-[#C9A800] text-[12px] font-semibold hover:underline underline-offset-4">
              Get a custom quote →
            </button>
          </motion.div>
            ...
            <div className="grid grid-cols-3 gap-px bg-white/[0.04] rounded-lg overflow-hidden">
              {fleet.map((s) => (
                <div key={s} className="bg-[#0a0a0a] hover:bg-[#111] transition-colors px-4 py-4 text-center">
                  ...
                  <span className="text-white/20 text-[10px] font-medium block mt-1">
                    {s === 4 ? 'Seater Car' : 'Seater'}
                  </span>
                </div>
              ))}
            </div>
            <div className="flex justify-center gap-4 mt-3 text-[10px] text-white/20">
