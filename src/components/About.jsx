    ...
    <section id="about" className="py-14 lg:py-20 border-t border-white/[0.04]">
      <div className="max-w-[1100px] mx-auto px-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16">
          ...
          >
            <p className="text-[#C9A800] text-[13px] font-semibold tracking-[0.25em] uppercase mb-3">Our Story</p>
          ...
          >
            {milestones.map((m, i) => (
              <div key={m.year} className={`flex gap-4 px-5 py-3.5 hover:bg-white/[0.02] transition-colors ${i < milestones.length - 1 ? 'border-b border-white/[0.04]' : ''}`}>
                <span className="text-[#C9A800] text-[13px] font-bold tracking-wide whitespace-nowrap w-28 shrink-0 pt-0.5">{m.year}</span>
                <p className="text-white/70 text-[14px] leading-relaxed">{m.text}</p>
              </div>
            ))}
          </motion.div>
        </div>
